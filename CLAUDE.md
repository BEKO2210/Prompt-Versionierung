...
| B1.2 | Real Anthropic adapter using `anthropic-dangerous-direct-browser-access` | next | streaming optional; non-stream first; falls back to mock when key missing |
| B1.3 | Real OpenAI adapter using `dangerouslyAllowBrowser` semantics | pending | mirrors B1.2 shape |
| B1.4 | Real Google Gemini adapter | pending | uses Gemini REST API directly |
